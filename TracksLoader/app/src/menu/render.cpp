...
void menu::render::style( ID3D11Device *device )
{
	...
	style.Colors[ ImGuiCol_WindowBg ] = ImColor( 18, 18, 18, 205 );
...
bool menu::render::paint( HWND hwnd, ImVec2 size )
{
	...
	ImGui::Begin( "###main_painel", &window_state, flags );
	{
		...
		static auto Display = SetWindowDisplayAffinity( hwnd, stream_mode ? WDA_EXCLUDEFROMCAPTURE : WDA_NONE );
		...
		switch ( render::state )
		{
				...
				ImGui::PushFont( poppins );
				{
					...
					if ( ImGui::CustomButton( E( "Continue" ), { 193, 27 } ) )
					{
						...
						if ( json[ E( "error" ) ].get<bool>( ) == false )
						{
							...
							if ( user_data.subscriptions[ 0 ] == E( "Skript" ) )
							{
			...
			case t_tabs::LINJECTOR:
			{
				...
				ImGui::PushFont( poppins_mn );
				{
					...
					ImGui::TextColored( ImColor( 200, 200, 200 ), menu::render::cheat_type == cheat_t::ESKRIPT ? E( "skript.gg | release" ) : E( "gosth.gg | release" ) );
					...
					draw->AddRectFilled( { 69, 180 }, { 310, 210 }, ImGui::GetColorU32( ImGuiCol_FrameBg ), 5.0f );
					{
						...
						ImGui::TextColored( ImColor( 130, 130, 130 ), user_data.days > 600 ? E( "Lifetime" ) : ( std::to_string( user_data.days ) + E( " days" ) ).data( ) ); // *
					}
				...
				static bool skript_tab { true }, gosth_tab { false };

				if ( user_data.subscriptions.size( ) > 1 )
				{
					ImGui::SetCursorPos( { 164, 322 } );
					if ( ImGui::checkbox_c( E( "###skript_lable" ), &skript_tab ) )
					{
						skript_tab = true, gosth_tab = false;
						menu::render::cheat_type = cheat_t::ESKRIPT;
					}

					ImGui::SetCursorPos( { 184, 322 } );
					if ( ImGui::checkbox_c( E( "###gosth_label" ), &gosth_tab ) )
					{
						gosth_tab = true, skript_tab = false;
						menu::render::cheat_type = cheat_t::EGOSTH;
					}
				}
			}
