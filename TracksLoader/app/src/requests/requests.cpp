...
network::response_t network::requests::perform_request( const std::string &method, const std::string &url, const std::string &body, const std::string &bearer_token ) const
{
    ...
    headers = curl_slist_append( headers, E( "Content-Type: application/json") );
    headers = curl_slist_append( headers, E( "User-Agent: Services/1.0.0 (24aeeb2b43db)") );
    headers = curl_slist_append( headers, ( E( "X-Api-Key: ") + std::string( api_key_ ) ).c_str( ) );

    if ( !bearer_token.empty( ) )
    {
        headers = curl_slist_append( headers, ( E( "Authorization: Bearer ") + bearer_token ).c_str( ) );
    }

    curl_easy_setopt( curl, CURLOPT_URL, url.c_str( ) );
    curl_easy_setopt( curl, CURLOPT_PINNEDPUBLICKEY, E( "sha256//fx0czMpWrXr/jRX1QqUwB/+DVJSS9xC69PcXnVKgcHY=") );
...
network::response_t network::requests::post_with_image( const std::string &path, const std::vector<uint8_t> &image_data, const nlohmann::json &json ) const
{
    ...
    curl_mime_name( part, E( "image") );
    curl_mime_filename( part, E( "image.png") );
    ...
    headers = curl_slist_append( headers, E( "User-Agent: Services/1.0.0 (24aeeb2b43db)") );
    headers = curl_slist_append( headers, ( std::string( E( "X-Api-Key: ") ) + api_key_ ).c_str( ) );
...
network::response_t network::requests::get( const std::string &path, const std::string &bearer_token ) const
{
    ...
    const auto response = perform_request( E( "GET"), base_url_ + path, "", bearer_token );
    log_dbg( E( "[network] request path /{} status {}"), path, response.status_code );
    return response;
...
network::response_t network::requests::post( const std::string &path, const nlohmann::json &json, const std::string &bearer_token ) const
{
    ...
    const auto response = perform_request( E( "POST"), base_url_ + path, json.dump( ), bearer_token );
    log_dbg( E( "[network] request path /{} status {}"), path, response.status_code );
    return response;
